...
article_compound = db.Table(
    ...
    db.Column("article_id", db.Integer, db.ForeignKey("article.id"), primary_key=True),
    db.Column(
        "compound_id", db.Integer, db.ForeignKey("compound.id"), primary_key=True
    ),
)
...
class Dataset(db.Model):
    ...
    articles = db.relationship(
        "Article", secondary=dataset_article, backref=db.backref("datasets", lazy=True)
    )
    training = db.Column(db.Integer, default=0)
    checker_dataset = db.relationship(
        "CheckerDataset", uselist=False, backref="dataset"
    )
    problems = db.relationship("Problem", backref="dataset")

    def get_articles(self):
        articles = Article.query.join(dataset_article).filter_by(dataset_id=self.id)
        return articles

    def get_compounds(self):
        compounds = []
        for art in self.articles:
            if not art.is_nparticle or art.needs_work:
                continue
    ...
    def standard_running(self):
        return (
            not self.checker_dataset.standardized and not self.checker_dataset.completed
            if self.checker_dataset
            else False
        )

    def checker_running(self):
        return (
            self.checker_dataset.standardized and self.checker_dataset.running
            if self.checker_dataset
            else False
        )
    ...
    def checker_completed(self):
        ...
        """
        return (
            self.checker_dataset.standardized and self.checker_dataset.completed
            if self.checker_dataset
            else False
        )
    ...
    def insert_errors(self):
        return self.checker_dataset.errors if self.checker_dataset else "No errors"
...
class Article(db.Model):
    ...
    compounds = db.relationship(
        "Compound", secondary=article_compound, backref="article"
    )
    # Tracking column
    completed = db.Column(db.Boolean, default=False)
    ...
    checker_article = db.relationship(
        "CheckerArticle", uselist=False, backref="article"
    )
...
class Compound(db.Model):
    ...
    checker_compound = db.relationship(
        "CheckerCompound", uselist=False, backref="compound"
    )
...
class CheckerCompound(db.Model):
    ...
    # Keep source genus even though taxon now
    source_genus = db.Column(db.String(255))
    source_species = db.Column(db.String(255))
    atlas_taxon_id = db.Column(db.Integer)
    npaid = db.Column(db.Integer)
...
class Taxon(db.Model):
    """Table of known altenatives to the given Taxon names in Atlas API
    ...
    atlas_taxon_id : int
        Id of known taxon
    alternatives : str
        Embedded string of alternatives to certified taxon
    """
    ...
    atlas_taxon_id = db.Column(db.Integer)
    # Column for holding alternatives, separated by special character @
    alternatives = db.Column(db.Text)

    @staticmethod
    def search_alternatives(query):
        return Taxon.query.filter(Taxon.alternatives.ilike(f"%@{query}@%")).first()
